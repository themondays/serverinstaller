/* Copyright (C) 2000-2001 Fujitsu Technology Solutions
   Joachim Braeuer
...
MODULE_AUTHOR("Markus Lyra");
MODULE_AUTHOR("Thomas Bretthauer");
MODULE_DESCRIPTION("FTS SmBios");
MODULE_LICENSE("GPL");

int major_number = 0;			 /* major number of the char-device */

static void *my_ioremap(unsigned long offset, unsigned long length)
{
    void *rv;

// Behaviour of ioremap was changed with 2.6.26
#if  (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 26))
    rv = ioremap_cache(offset, length);
    if (!rv)
#endif
        rv = ioremap(offset, length);
    return rv;
}
...
init_module (void)
{
    ...
    unsigned long PhysSmbiosStruct = 0;

    PDEBUG ("starting module initialization\n");

#ifdef CONFIG_EFI
    if (efi_enabled && efi.smbios != EFI_INVALID_TABLE_ADDR)
    {
        PhysSmbiosStruct = (unsigned long)efi.smbios;
    }
    else
#endif
    {
         ...
         */
        if (!(smbios_base = my_ioremap (BIOS_START_ADDRESS, BIOS_MAP_LENGTH)))
        {
         ...
         */
        Offset = smbios_find_entry_point (smbios_base);
        if (Offset < 0)
        {
        ...
        PhysSmbiosStruct = BIOS_START_ADDRESS + Offset;
    }
    /*
     *  for SM-BIOS:
    ...
    if (PhysSmbiosStruct)
    {
        if (!(smbios_entry_point = my_ioremap (PhysSmbiosStruct, sizeof(smbios_entry_point_struct))))
        {
            PDEBUG ("ioremap() for UEFI entry point failed\n");
            err = -ENXIO;
            goto ioremap_for_entry_point_failed;
        }
        if (strncmp((char *) &(smbios_entry_point->intermediate_string), DMI_STRING, sizeof (DMI_STRING) - 1))
        {
             ...
             */
            if (!(smbios_structures_base = my_ioremap(smbios_entry_point->struct_table_address, smbios_entry_point->struct_table_length)))
            {
    ...
    if (dmibios_entry_point)
    {
        ...
        {
            PDEBUG ("invalid structure table entry%p,%p\n", smbios_structures_base, dmibios_entry_point->entry);
            err = -ENXIO;
            goto ioremap_for_structures_table_failed;
        }
    }
    ...
    if (!(smbios_proc_dir = create_proc_entry (PROC_DIR_STRING, S_IFDIR, NULL)))

#else /* Linux version >= 3.10 */

    if (!(smbios_proc_dir = proc_mkdir(PROC_DIR_STRING, NULL)))
    ...
    if (!(smbios_raw_proc_dir = create_proc_entry (PROC_DIR_STRING_RAW, S_IFDIR, smbios_proc_dir)))

#else /* Linux version >= 3.10 */

    if(!(smbios_raw_proc_dir = proc_mkdir(PROC_DIR_STRING_RAW, smbios_proc_dir)))
    ...
    if (!(smbios_cooked_proc_dir = create_proc_entry (PROC_DIR_STRING_COOKED, S_IFDIR, smbios_proc_dir)))

#else /* Linux verison >= 3.10 */

    if (!(smbios_cooked_proc_dir = proc_mkdir(PROC_DIR_STRING_COOKED, smbios_proc_dir)))
...
create_smbios_raw_dir_failed:
    ...
    remove_proc_entry(PROC_DIR_STRING, NULL);
...
//    These are allready removed by smbios_destroy_dir_entries (smbios_proc_dir);
//    remove_proc_entry(PROC_DIR_STRING_COOKED, smbios_proc_dir);
//    remove_proc_entry(PROC_DIR_STRING_RAW, smbios_proc_dir);

#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)

    /* in Kernel version >= 3.10 the smbios_destroy_dir_entries above
     * already kills the smbios_proc_dir by issueing a proc_remove for the whole dir.
     * So if we would do this in a Kernel >=3.10 we would get an Oops.
     */
